...
| `tools/` | 可重跑的證據管線與維護腳本 | 一次性的分析腳本（走 scratchpad） |
...
- 繁體中文是正本；`*.en.md`／`*.ja.md` 是翻譯，**改中文的同輪要改翻譯**，
  不然三份會慢慢分岔而且沒有人知道哪一份是對的。
- 程式碼、助記符、識別字、檔名保留原文。
- 標點全形（`，。：；（）`）。半形只用在三處：程式碼區塊內、行內程式碼內、
  英文列舉（`DB, B`）與英文原名（`Cursor X,Y Positioning`）。
  用 `tools/normalize-punct.py` 統一；**新增引用的英文原文時要同時加進該檔的 `KEEP` 清單**，
  否則下一次重跑會把它的標點也轉掉。
...
- **查詢落空不等於不存在。** grep 沒中先換大小寫、換編碼、拆字再搜，並做一次正對照
  （拿已知會中的樣本驗證搜尋本身沒壞）。
- **接手別人的產出，先驗完整性再驗形式。** 一份文件宣稱涵蓋什麼範圍，就去核對它是不是
  真的寫到那裡——標點、連結、索引這些形式檢查全部通過，不代表內容沒有斷在一半。
  同一批產出擺在一起時，**篇幅的異常值就是第一個訊號**。
