...
### R10（2026-09-06）`EXIT` 的拆框機制

手冊在 `RPU` 那一條末尾補的一句「若 MSCW 裡的程序號 < 0，返回該程序的
`EXITIC`」背後是一整套機制：`EXIT(proc)` 一次退出好幾層的做法，
是把沿路每一格 Mark Stack 的 `MSPROC` **取負**——負號就是記號。
返回時看到負號就跳那支程序的離場碼，一路退到底，而且每一層的收尾都跑得到。

8086 版還多一道：**返回點與 `EXITIC` 取比較後面的那一個**，
已經走過離場碼就不再跳回去，否則收尾會跑兩次。
第三條路是 `MSIPC` 為 0——呼叫者是原生碼，控制權要交回機器碼那一側。

三條路只差兩個 word 的符號與零。這一條同時解釋了 `EXITIC` 為什麼存在，
以及它為什麼擺在字典項**前面**一個 word。

姊妹 repo 的重做在這一輪開到了命令列，逐支原生程序與多工的細節寫在那邊。
