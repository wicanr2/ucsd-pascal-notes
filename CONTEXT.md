...
| **Pack-ptr** | packed 欄位的「位址」：堆疊上三個 word——最右 bit 編號、位元數、word 位址 |
| **PME（P-Machine Emulator）** | IV.x 對直譯器的稱呼。發行磁碟上的檔名帶 CPU 後綴，如 `SYSTEM.PME.86` 是 8086 版 |
| **JTAB** | I.5 的程序資訊表。低位元組是程序號、高位元組是 lex level；IV.0 用程序編號取代它 |
