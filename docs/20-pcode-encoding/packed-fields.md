...
- 上面三段碼來自 SunDog 的 IV.2.1 直譯器。IV.0 手冊只定義語意，不規定實作手法——
  **同一版的 8086 直譯器就是用查表**：`and ax, cs:[bp+1FB6h]`，`0x1fb6` 起是一張
  `0x007f, 0x00ff, 0x01ff, 0x03ff…` 的遮罩表，欄位寬度當索引。
  兩者都在避開「執行期算遮罩」，手段不同——見
  [同一版、兩個 CPU](../30-opcode-tables/iv21-two-cpus.md)。
