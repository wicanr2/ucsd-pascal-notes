...
手冊在 `RPU` 那一條的末尾補了一句：「若 MSCW 裡的程序號 < 0，
返回該程序的 `EXITIC` 而不是 MSCW 的 `IPC`」（p.67）。那句話背後是一整套機制。

Pascal 的 `EXIT(proc)` 可以**一次退出好幾層**。做法不是去展開堆疊，
而是把沿路每一格 Mark Stack 的 `MSPROC` **取負**——負號就是記號。
返回時看到負的程序號，就不回呼叫點，改跳到那支程序自己的**離場碼**
（`EXITIC`，段內位元組偏移，放在常式碼開頭 `DATASIZE` 前面一個 word）。
每一層都這樣，於是一路退到底，而且每一層的收尾程式碼都跑得到。

8086 版還多做一件事（@0x118E）：**返回點與 `EXITIC` 取比較後面的那一個**。
已經走過離場碼就不要再跳回去，否則收尾會跑兩次。

第三條路是 `MSIPC` 為 0：那表示呼叫這一格的是原生碼，
p-code 這一側沒有位址可跳，只能把控制權交回機器碼那一側
（8086 版是一個 `lret`）。

所以 `RPU` 的三條路只差兩個 word 的符號與零：

| MSPROC | MSIPC | 跳到哪 |
|---|---|---|
| 正 | 非 0 | 呼叫點（`MSIPC`）|
| **負** | 非 0 | 那支程序的 `EXITIC`，與 `MSIPC` 取較後面的 |
| 正 | **0** | 交回原生碼 |

搞錯不會當場報錯，只會跳到別的地方去執行。
