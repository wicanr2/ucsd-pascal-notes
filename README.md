...
反組譯，含分派機制、狀態模型、定址、段切換、段 1 的內嵌原生程序、多工與換 task，
以及 256 格對照表——並在同一個 repo 裡用 Go 重做一台。

**那台重做的機器已經開得起來**：一片 1984 年的 `.VOL` 磁碟映像丟進去，
p-System 自己開到命令列，按鍵有反應，Filer 列得出目錄；整段開機的
226,623 條 p-code 與原版逐條相同。

**通則、編碼原理與手冊摘譯在這裡；8086 那一份實作的細節與重做在那裡。**
